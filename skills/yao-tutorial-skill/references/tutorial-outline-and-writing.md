...
### Minutes/Transcript-Sourced Tutorial Additional Checks

When the tutorial originates from a transcript or Feishu Minutes:

- [ ] **No S2T errors in output**: every file extension, product name, API path, and technical term has been verified against official docs (not the transcript)
- [ ] **Structure matches information density**: no section is inflated to match another section's depth when the source content is thinner
- [ ] **All operation paths are complete**: entry → steps → result, no vague "go to settings" without specifying where
- [ ] **All links are real and clickable**: no placeholder like "see documentation" or "the link in the reference"
- [ ] **Screenshot positions marked with semantic descriptions**: every UI workflow step has a `【这里需要放一张XXX界面的截图】` note where a screenshot should appear, e.g., `【这里需要放一张飞书开发后台-新建智能体页面的截图】`
- [ ] **Multiple approaches listed**: when the speaker mentions more than one way, all ways appear in the tutorial
- [ ] **Speaker mistakes are prominent warnings**: "I forgot to do X and had to redo it 4 times" → bold warning box, not a passing mention
- [ ] **Title framing is honest**: "经验" "实践笔记" "避坑指南" for single-session transcripts; "教程" "通关指南" only when content genuinely supports comprehensive scope
- [ ] **Scope claims verified**: "6-10题都一样" → check actual range; do not repeat unverified generalizations
